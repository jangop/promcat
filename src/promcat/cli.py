...
def format_file_section(
    ...
    formatted_content = add_line_numbers(content, separator) if line_numbers else content
    ...
    if style == HeaderStyle.NEWLINE:
        ...
        return f"\n<file><path>{path}</path><content>{formatted_content}</content></file>"
        
    return formatted_content  # fallback case

def is_text_file(filepath: Path) -> bool:
    """Check if a file is likely a text file based on content sampling."""
    try:
...
def get_gitignore_matcher(root_dir: Path) -> Optional[callable]:
    """Create a gitignore matcher function if .gitignore exists."""
...
def collect_text_files(
    ...
    for path in root_dir.rglob('*'):
        ...
        if respect_gitignore and gitignore_matcher:
...
@click.command()
@click.argument('directory', type=click.Path(exists=True, path_type=Path), default=Path.cwd())
@click.option('--output', '-o', type=click.Path(path_type=Path), 
              help='Output file path (defaults to stdout)')
@click.option('--respect-gitignore/--no-respect-gitignore', default=False,
...
@click.option('--style', type=click.Choice(['newline', 'separator', 'markdown', 'xml']),
              default='separator', help='Header style to use')
@click.option('--footer/--no-footer', default=False,
              help='Include footers for file sections')
@click.option('--line-numbers/--no-line-numbers', default=False,
...
def main(
    ...
    """Concatenate all text files in a directory, optionally respecting .gitignore"""
    gitignore_matcher = get_gitignore_matcher(directory) if respect_gitignore else None
    text_files = collect_text_files(directory, respect_gitignore, gitignore_matcher)
    
    result = []
    for file_path in text_files:
        try:
            path_str = str(file_path.relative_to(directory)) if relative else str(file_path)
